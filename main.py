...
class Screen:
    ...
    def menu(self):
        ...
        self.display.blit(FONT2.render('STANDARD WITH BOT', True, GREY), self.text_menu3_rect)
        self.display.blit(FONT2.render('CUSTOM WITH BOT', True, GREY), self.text_menu4_rect)
        self.display.blit(FONT1.render('PING', True, RED),
                          (self.width - 300, (self.height - self.text_menu5_size[1] - 52) // 2))
        self.display.blit(FONT1.render('PONG', True, BLUE),
                          (self.width - 320, (self.height - self.text_menu6_size[1] + 52) // 2))

    def blit_item(self, ball, player1, player2):
        self.display.fill(BLACK)
        self.display.blit(FONT3.render(str(player1.score), True, RED), (self.width // 2 - 30, 10))
        self.display.blit(FONT3.render(str(player2.score), True, BLUE), (self.width // 2 + 30, 10))
    ...
    def end_game(self):
        ...
        if self.player_blue_win:
            self.display.blit(FONT1.render('BLUE', True, BLUE), (0, self.height - FONT1.get_height() + 7))
        ...
        if self.player_blue_win:
            self.display.blit(FONT1.render('WIN', True, WHITE), (self.width - FONT1.size('WIN')[0], 0))
        elif self.player_red_win:
            self.display.blit(FONT1.render('WIN', True, WHITE),
                              (self.width - FONT1.size('WIN')[0], self.height - FONT1.get_height() + 7))

        if self.end_text2:
            pygame.display.flip()
            time.sleep(0.5)
            self.end_text2 = False
        self.display.blit(FONT2.render('ENTER TO RESTART', True, GREY), (
            (self.width - FONT2.size('ENTER TO RESTART')[0]) // 2, (self.height - FONT2.get_height() - 50) // 2))
        self.display.blit(FONT2.render('ESC TO MENU', True, GREY), (
            (self.width - FONT2.size('ESC TO MENU')[0]) // 2, (self.height - FONT2.get_height() + 50) // 2))
    ...
    while menu_on:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (menu_screen.text_menu1_rect[0] <= event.pos[0] <= menu_screen.text_menu1_rect[0] +
                        menu_screen.text_menu1_size[0]
                        and menu_screen.text_menu1_rect[1] <= event.pos[1] <= menu_screen.text_menu1_rect[1] +
                        menu_screen.text_menu1_size[1]):
                    menu_screen.sound_menu.play()
                    menu_on = False
                    main('STANDARD')
                elif (menu_screen.text_menu2_rect[0] <= event.pos[0] <= menu_screen.text_menu2_rect[0] +
                      menu_screen.text_menu2_size[0]
                      and menu_screen.text_menu2_rect[1] <= event.pos[1] <= menu_screen.text_menu2_rect[1] +
                      menu_screen.text_menu2_size[1]):
                    menu_screen.sound_menu.play()
                    menu_on = False
                    main('CUSTOM')
                elif (menu_screen.text_menu3_rect[0] <= event.pos[0] <= menu_screen.text_menu3_rect[0] +
                      menu_screen.text_menu3_size[0]
                      and menu_screen.text_menu3_rect[1] <= event.pos[1] <= menu_screen.text_menu3_rect[1] +
                      menu_screen.text_menu3_size[1]):
                    menu_screen.sound_menu.play()
                    menu_on = False
                    main('STANDARD', True)
                elif (menu_screen.text_menu4_rect[0] <= event.pos[0] <= menu_screen.text_menu4_rect[0] +
                      menu_screen.text_menu4_size[0]
                      and menu_screen.text_menu4_rect[1] <= event.pos[1] <= menu_screen.text_menu4_rect[1] +
                      menu_screen.text_menu4_size[1]):
                    menu_screen.sound_menu.play()
                    menu_on = False
                    main('CUSTOM', True)
